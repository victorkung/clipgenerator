...
- **Center:** source head, pane tabs (Transcript | Captions | Agent handoff), paper, hint row.
- **Craft:** monitor 158px, source ruler 44px, Mark, Export, The post (`flex:1`, paper textarea min 88px).
...
`paneTab`: `'transcript' | 'captions' | 'agent'`

- Agent tab only when `CLIPGENERATOR_AGENT_FLOW` is on.
- Flag off → two tabs; layout otherwise unchanged.
