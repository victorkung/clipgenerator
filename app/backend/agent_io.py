...
def segments_to_transcript_compact(segments: list[dict[str, Any]] | None) -> str:
    """One `[m:ss] text` line per cue. For in-app LLM calls.

    The verbose markdown form duplicates start/end in two formats and a blank
    line per cue, which blows past the model cap on multi-hour lives.
    """
    lines: list[str] = [
        "# Transcript",
        "",
    ...
    for seg in segments or []:
        ...
        if not text:
            continue
        start = float(seg.get("start") or 0.0)
        lines.append(f"[{format_ts_label(start)}] {text}")
        n += 1
    if n == 0:
        lines.append("_(no segments)_")
    return "\n".join(lines).rstrip() + "\n"
...
def build_clip_reference_md(source: dict[str, Any]) -> str:
    """Clip agent: source meta + pointer to pasted Summary agent output."""
    has_summary = bool((source.get("summary_post_text") or "").strip())
    summary_line = (
        "see `04-summary.md` (pasted Summary agent output)"
        if has_summary
        else "_(missing — required for clip export)_"
    )
    # Optional legacy URL if still stored on the source
    summary_url = (source.get("summary_post_url") or "").strip()
    url_lines: list[str] = []
    if summary_url:
        url_lines = [
            f"**Summary post URL (optional quote target):** {summary_url}",
            "",
        ]
    return "\n".join(
        [
            ...
            *_meta_lines(source),
            "",
            f"**Summary post draft:** {summary_line}",
            "",
            *url_lines,
            "Open the **Source URL** for episode context and handles.",
            "Use `04-summary.md` as editorial context for the thread (opener tee-up,",
            "what the recap already covered, tone). Prefer quoting that summary when",
            "the human posts the thread; do not re-draft a full second recap.",
            "",
        ]
    )
...
def build_clip_prompt_md(source: dict[str, Any]) -> str:
    ...
    has_summary = bool((source.get("summary_post_text") or "").strip())
    summary_block = (
        "included as `04-summary.md`"
        if has_summary
        else "_(paste Summary agent output into clipgenerator before export)_"
    )

    return "\n".join(
        [
            "# Clip plan — replies in the thread",
            "",
            "clipgenerator packaged this episode for an external clip agent.",
            "",
            f"**Episode length:** {length_note}",
            f"**Thread opener (Summary agent):** {summary_block}",
            "",
            ...
            "- `04-summary.md` — thread opener from the Summary agent (first post of the thread)",
            "",
            "## How this ships",
            "",
            "Not: long summary + each clip quoting it as a separate standalone.",
            "`04-summary.md` is the **initial post of a thread**. Find clips that become",
            "**additional replies in that same thread**. Each clip = its own post.",
            "Goal: easy way to learn the lessons from the transcript.",
            "",
            "## Selection + narrative",
            "",
            "- Hit the tldrs / themes in `04-summary.md`",
            "- Prefer actionable takeaways + strong walkthroughs / worked examples when present",
            "- Think narrative for captions and for **order** of posts",
            "- Usual workflow: shortlist → select → one clip at a time",
            "",
            "Import JSON = **video clips only** (not the opener), in thread order.",
...
def build_clip_summary_md(source: dict[str, Any]) -> str:
    """Paste of the Summary agent's output for the clip / thread agent."""
    body = (source.get("summary_post_text") or "").strip()
    if body:
        content = body
    else:
        content = (
            "_(Paste the Summary agent output in clipgenerator Agent handoff "
            "before exporting the clip package.)_"
        )
    return "\n".join(
        [
            "# Thread opener (Summary agent output)",
            "",
            "This is post 1 of the thread — the teaser drafted by the Summary agent.",
            "Clips you propose become replies under this post. Do not re-write it.",
            "",
            content,
            "",
        ]
...
def build_summary_prompt_md(source: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Thread opener (summary agent)",
            "",
            "clipgenerator already transcribed the source and packaged the files below.",
            ...
            "## What to produce",
            "",
            "Ship a **thread**. Post 1 is an **initial teaser** for later clip posts",
            "(handled by the clip agent).",
            "",
            "Modify the usual summary-post structure. For this opener only:",
            "1. title / hook",
            "2. intro (tight; tag main people)",
            "3. tldrs — thematic (**not chronological**), a bit meatier; short sections ok",
            "4. call-to-action — tell the reader to keep reading the thread for the clips",
            "",
            "Do not write the long key-takeaways essay in this post. Leave depth for clip replies.",
...
def write_clip_export(
    ...
    *,
    ...
    """Write agent-export/clip/ package only. Requires summary_post_text on source."""
    summary_text = (source.get("summary_post_text") or "").strip()
    if not summary_text:
        raise ValueError(
            "summary_post_text is required for clip export — paste the Summary agent output first"
        )
    root = project_dir / "agent-export"
    out_dir = root / "clip"
    transcript = segments_to_transcript_md(segments)
    files = _write_dir(
        out_dir,
        {
            ...
            "03-transcript.md": transcript,
            "04-summary.md": build_clip_summary_md(source),
        },
    )
...
def write_agent_export(
    ...
    *,
    ...
    """Write both packages (legacy / bulk). Clip package skipped if no summary text."""
