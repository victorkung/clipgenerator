...
class Library:
    ...
    def get_lessons(self, field: str) -> list[str]:
        with _lock:
            raw = self._read().get(field) or []
        return [str(x).strip() for x in raw if str(x).strip()]

    def merge_lessons(self, field: str, new: list[str], *, cap: int = 24) -> list[str]:
        """Prepend new lessons, drop duplicates, keep the newest first."""
        incoming = [str(x).strip() for x in (new or []) if str(x).strip()]
        with _lock:
            data = self._read()
            old = [
                str(x).strip()
    ...
    def get_writer_lessons(self) -> list[str]:
        return self.get_lessons("writer_lessons")

    def merge_writer_lessons(self, new: list[str], *, cap: int = 24) -> list[str]:
        return self.merge_lessons("writer_lessons", new, cap=cap)

    def get_reply_lessons(self) -> list[str]:
        return self.get_lessons("reply_lessons")

    def merge_reply_lessons(self, new: list[str], *, cap: int = 24) -> list[str]:
        return self.merge_lessons("reply_lessons", new, cap=cap)
...
def make_source(
    *,
    ...
    return {
        ...
        "error": None,
        "model": None,
        # Pipeline analytics (wall times per stage; see main._pipeline_* helpers)
        # {started_at, finished_at, model, audio_duration_s, stages:{…}, total_s,
        #  stt_realtime_factor, ok, note?}
        "pipeline": None,
        ...
        "summary_post_url": None,  # legacy optional X URL (no longer required)
        "summary_post_text": None,  # Summary opener (pasted or in-app draft)
        "agent_run": None,  # {summary:{status,draft,approved,...}} in-app runner
        "clips": [],
        "created_at": now,
        "updated_at": now,
    }
